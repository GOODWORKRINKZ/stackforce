...
│  (Передняя часть)   │    1 Mbps               │  (Задняя часть)    │
└─────────────────────┘                         └────────────────────┘
         │                                                 │
    ┌────┴────┐                                      ┌────┴────┐
    │PPM + IMU│                                      │         │
...
### 1. ESP32 Main (Главный контроллер - передняя часть)

**Функции:**
- ✅ Прием PPM сигнала от RC приемника (8 каналов)
- ✅ Чтение IMU (MPU6050) с комплементарным фильтром
- ✅ Управление 2 передними BLDC моторами через CAN
- ✅ Управление 4 передними сервоприводами (2 ноги) через PCA9685
- ✅ **Кинематика ног** (прямая и обратная)
- ✅ **Генератор походок** (STAND, WALK, TROT, BOUND)
- ✅ Отправка команд aux контроллеру по CAN

### 2. ESP32 Aux (Вспомогательный контроллер - задняя часть)

**Функции:**
- ✅ Прием команд от main контроллера по CAN
- ✅ Управление 2 задними BLDC моторами через CAN
- ✅ Управление 4 задними сервоприводами (2 ноги) через PCA9685
