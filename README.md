...
Полная прошивка для **четырехногого колесного робота** с использованием **двух ESP32-S3 контроллеров** (main и aux), связанных через CAN шину.

> 📺 **Основано на открытых уроках 登哥 (Deng Ge)** - модульная платформа, объединяющая преимущества колесного и ножного движения.

**Наш подход:** Гибридная колесно-ножная платформа с приоритетом на колесное движение и статическую адаптацию высоты ног.
...
#### Автоматическая калибровка офсетов (НОВОЕ!)

Используйте специальную прошивку для автоматического определения офсетов:
...
**Прошивка калибровки позволяет:**
- 📊 **Непрерывный вывод** текущих углов и офсетов в консоль (каждые 100 мс)
- 🎮 **Управление через Serial** - установка углов отдельных серво или всех сразу
- 📝 **Автоматический расчет** офсетов относительно нейтральной позиции (90°)
- 💾 **Готовый код** для вставки в config.h основной прошивки

**Команды Serial:**
```
s0 90        # Установить серво 0 в 90°
a 90         # Установить ВСЕ серво в 90°
n            # Нейтральная позиция (90°)
h            # Справка
```
...
Подробная инструкция: [firmware/servo_calibration/README.md](firmware/servo_calibration/README.md)
...
│   └── platformio.ini
│
└── servo_calibration/        # 🆕 Калибровка офсетов сервоприводов
    ├── include/
    │   └── config.h         # Конфигурация калибровки
    ├── src/
    │   └── main.cpp         # Прошивка калибровки
...
- [TWAI Driver (CAN для ESP32-S3)](https://docs.espressif.com/projects/esp-idf/en/latest/esp32s3/api-reference/peripherals/twai.html)

---

## 🎓 Теория самостабилизации

Робот использует **каскадный PID** для адаптации к сложному рельефу (лестницы, неровности).

### Принцип работы:

```
1. IMU обнаруживает наклон (pitch/roll)
   ↓
2. PID вычисляет целевую угловую скорость
   ↓
3. PID вычисляет скорость изменения высоты ног
   ↓
4. Высота ног накапливается (интегрируется)
   ↓
5. Робот стабилизируется
```

**Ключевая формула** (из видео 登哥):

```cpp
// Ошибка угла → целевая угловая скорость
target_velocity = Kp_angle * (0 - current_angle);

// Ошибка скорости → приращение высоты
...
Подробности в [docs/STABILIZATION_THEORY.md](docs/STABILIZATION_THEORY.md)

### Наша реализация:

- ✅ Каскадный PID для pitch (скорость → угол → момент)
- ✅ Инкрементальная коррекция крена `stab_roll`
- ✅ Асимметричное управление высотой L/R ног
- ✅ Частота обновления 200 Hz
- ✅ Обработка pitch и roll одновременно

### Отличие от классической схемы:

**Видео (登哥):** Прямая регулировка высоты Y через PID  
**Наш код:** Комбинация момента моторов + адаптация высоты ног

Это гибридный подход для лучшей производительности на скорости!

---

## 🚶 Походки (Gaits) - НЕ РЕАЛИЗОВАНО

### Что такое походки?

**Походка** - это периодическая координация 4 ног для стабильного ножного движения. Без походки робот упадет при попытке шагать.

### Типы походок из видео:

| Походка | Описание | Применение |
|---------|----------|-----------|
| **Trot** (交叉步态) | Диагональные пары ног | Стабильность + скорость |
| **Bound** (跳跃步态) | Передние/задние пары | Высокая скорость (галоп) |
| **Pace** (同侧步态) | Левая/правая пары | Специальные случаи |

### Циклоидная траектория

Математика из урока 4 (登哥):

**Swing Phase (нога в воздухе):**
```cpp
X_swing = -S/2 + S*σ - (S/(2π)) * sin(2π*σ)
Y_swing = H_max - H + H * (1 - cos(2π*σ))
```

**Stance Phase (нога на земле):**
```cpp
X_stance = S/2 - S*σ
Y_stance = H_max
```

Где: `σ = (t % T_s) / T_s` (фаза 0..1), `S` = амплитуда шага, `H` = высота подъема

### Наш статус

**❌ У нас НЕТ походок!**

Наш код - **гибридный колесно-ножный**:
- ✅ Колеса для основного движения (моторы)
- ✅ Ноги для статической адаптации высоты
- ❌ Ноги НЕ чередуются циклически
- ❌ Все 4 ноги синхронизированы (не Trot/Bound)

Подробный анализ и готовые формулы: [docs/GAIT_THEORY.md](docs/GAIT_THEORY.md)

**Когда нужно добавить походки:**
- Для чистого ножного движения без колес
- Для перешагивания больших препятствий
- Для имитации животного
...
- **[ДЕТЯМ_О_РОБОТЕ.md](docs/ДЕТЯМ_О_РОБОТЕ.md)** - 🎓 Простое объяснение для детей (9+ лет): как работает робот, обратная кинематика, стабилизация, походки, коммуникация

### Теоретические основы (переводы видео 登哥)

- **[OVERVIEW.md](docs/OVERVIEW.md)** - Обзор платформы и три режима движения (урок 1)
- **[STABILIZATION_THEORY.md](docs/STABILIZATION_THEORY.md)** - Теория самостабилизации (урок 5)
- **[GAIT_THEORY.md](docs/GAIT_THEORY.md)** - Теория походок и циклоидных траекторий (урок 4)
...
- Колесный режим (высокая скорость)
- Стабилизация pitch/roll (каскадный PID)
- Обратная кинематика (5-звенный механизм)
- Адаптация высоты ног по крену
- SBUS RC управление

**❌ НЕ реализовано:**
- Ножной режим (походки Trot/Bound/Pace)
- Гибридный режим (колеса + циклические походки)
- Циклоидные траектории стоп
- Индивидуальное управление 4 ногами

**⚠️ Наш подход:**
Гибридная колесно-ножная платформа с приоритетом на колесное движение и статическую (не циклическую) адаптацию высоты ног.
