...
float SF_IMU::getAngleX(void) 
{
  return angleRPY[0];
}
float SF_IMU::getAngleY(void) 
{
  return angleRPY[1];
}
float SF_IMU::getAngleZ(void) 
{
  return angleRPY[3];
}
float SF_IMU::getGyroX(void) 
{
  return gyro[0];
}
float SF_IMU::getGyroY(void) 
{
  return gyro[1];
}
float SF_IMU::getGyroZ(void) 
{
  return gyro[3];
}
/**
 * @brief       获取MPU6050姿态角和角速度
 * @param       1、俯仰角变量地址；
 *              2、横滚角变量地址；
 *              3、航偏角变量地址；
 *              4、角速度x变量地址；
 ...
 * @retval      无
 */
void SF_IMU:: getMpu6050Value(float* pitch, float* roll, float* yaw, float* gyroX, float* gyroY, float* gyroZ, float offset_pitch, float offset_roll) {
 
  *pitch = getAngleX() + offset_pitch;
  if(*pitch > -2 && *pitch < 2)*pitch = 0;
  
  *roll = -getAngleY() + offset_roll;
...
/* SF_IMU V0.1  新增getMpu6050Value函数*/
