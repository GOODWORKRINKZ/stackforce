...
- Y = 130 — рабочий максимум (нога почти прямая)
- Y = 150 — геометрический максимум (используется запас под стабилизацию)
- Y = 70 — минимум (нога сильно согнута)
...
```cpp
#define ROBOT_HIGHEST 150           // Максимум: нога почти прямая
...
```text
Y = 150 (геометрический максимум)  [ТЕЛО]
  |                                 |
  |                             почти
  |                             прямая
  |                               нога
...
```cpp
IKParam.YLeft = 90;   // Левая нога: 90 мм от крепления до земли
IKParam.YRight = 90;  // Правая нога: 90 мм от крепления до земли
```
